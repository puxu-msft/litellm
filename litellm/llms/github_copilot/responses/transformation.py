...
def github_copilot_supports_responses_api(model: str) -> bool:
    ...
    Resolution (first match wins): mode "responses" -> True; mode "chat" or
    "anthropic" -> False (opt-out / force-messages win for dual-endpoint models);
    else the model's dynamically resolved endpoints contain "responses". Unknown
    model -> False (the chat/completions bridge always works).

    ``mode`` is read from the merged model info (per-deployment model_info applied
    via ``register_model``); endpoint capability is read from the shared resolver
    (live upstream /models cache, falling back to the raw model_cost entry).
    """
    ...
    mode = forced_mode(info if isinstance(info, dict) else None)
    if mode == "responses":
        return True
