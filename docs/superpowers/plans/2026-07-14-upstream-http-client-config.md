...
> **STATUS (2026-07-17): ALL PHASES COMPLETE and committed on branch `ghc`.** All 36 task units (28 numbered + 8 lettered: 4a/7a/8a/16a/18a/20a/21a/24a) are done — the full config surface (Phase 1), the unified asyncio deadline infrastructure (`asyncio_deadline.py`, `DeadlineExceeded`→`litellm.Timeout` mapping) (Phase 2), chat/responses/messages deadline wiring incl. streaming phase①/② and Router mid-stream fallback (Phases 3-4), and Router regression + custom-client bypass warnings across all 4 choke points + http2 schema-only + mutation testing (Phase 5). Task 28 mutation run: **123/125 = 98.4% kill** on `http_client_config.py` + `asyncio_deadline.py` (the 2 survivors are provably equivalent mutants). Follow-on tree cleanup after the plan: the http_client feature's dashboard API-type footprint was regenerated (`schema.d.ts`), and the LIT006 type-discipline budget casts were refactored/justified back under ceiling. The kick-off block below is retained for history only — there is nothing left to execute. Deviations from the plan are recorded in the "Open Items for Implementer" section and the commit history (e.g. several tests assert the true mapped exception types rather than raw `DeadlineExceeded`; the github_copilot end-to-end no-leak tests pivoted to a direct-handler capture harness because github_copilot bridges via the OpenAI SDK, not `AsyncHTTPHandler.post`).
