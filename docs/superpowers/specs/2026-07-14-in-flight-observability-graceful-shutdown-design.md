...
**不让通用 `LoggingWorker` 依赖 proxy 类型。** `LoggingTask` 从 mutable `TypedDict` 改为 `frozen dataclass(slots=True)`，新增可选中立字段 `token: CompletionToken | None`（`CompletionToken` 是中立 protocol，`token.settle()` 幂等；proxy 的 `AccountingLease` 实现它，纯 SDK 传 `None`）。token 只表达 queue item 已终结；`Drained | DeadlineExceeded | ForcedExit` outcome 由 worker/supervisor 的 drain 结果在高层表达，不作为 token 参数。`ensure_initialized_and_enqueue` 加 keyword-only `token=None`。token 随 task 转交，且只在 callback 真正结束或 task 被确定丢弃时 settle；`enqueue` 无 queue、retry 无 loop/无 queue、loop rebind、atexit 达限剩余项等**每一条 drop/rebind 路径都必须 close 未执行 coroutine 并 settle token**，否则 record 泄漏。SDK（`token=None`）路径行为保持不变，加纯 SDK 回归测试守此。
...
**LoggingWorker 加独立 `quiesce(*, deadline_remaining, is_force_exit, admission_policy, downstream_is_quiescent) -> LoggingDrainOutcome`，不复用 `flush()`/`stop()`**（现有 `flush()` 只 `await queue.join()`、不冻结 admission、不取消；`stop()` 的取消路径会调 `clear_queue()`，可能在 deadline 后仍执行 queued coroutine，正是 round-2 blocker 复发点）。`is_force_exit` 是区分 `ForcedExit` 与 `DeadlineExceeded` 的独立输入，不能从 deadline 推断。`quiesce` 契约：关闭 proxy root logging admission（SDK 非 proxy admission 是否续由调用方决定）；正常阶段让已登记 item 执行；deadline 后**不再执行 queued coroutine**，而是逐项 close coroutine、settle token、`task_done`；cancel worker processing/retry/aggressive-clear 并 await settlement；**禁止取消路径再调 `clear_queue()` 执行业务 callback**。fixed-point 通过中立 `downstream_is_quiescent` 观察回调联合检查 LoggingWorker 与 supervisor：queue unfinished、processing/retry/aggressive-clear helper、supervisor task/admission 全空，并在一个 loop tick 后复检稳定才算 `Drained`；单纯串行一次 `queue.join()` 再一次 supervisor drain 会漏掉两次检查之间的新 work。现有 `GLOBAL_LOGGING_WORKER.flush()` 全仓仅 3 处测试调用，故普通 `flush()` 语义保持不变、只新增 proxy quiesce 路径。
...
`InFlightRegistry` 为**内部原子 reducer**（单事件循环、方法内同步无 await，mutation 与向 subscriber 发布之间不 await）：调用者只传 typed intent、不传 expected version。`register` / `advance_stage`（有序 transition 或 `max(current, requested)`，terminal 后拒更新）/ `set_llm_context`（只替换自有字段）/ `acquire_lease` / `release_lease` / `snapshot() -> tuple[RequestRecord, ...]`；每次成功 mutation 递增 record `version` + global event sequence。terminal reason 明确 `completed | failed | cancelled | timed_out | shutdown_dropped`。**不**暴露 CAS 给调用者（若确需跨线程乐观并发，CAS API 须返回 `Applied | Conflict | Missing | Terminal` tagged union 由上层重试，不返 None 静默丢）。

终端可观测 collector 的关停位置由 [terminal-observability Spec](./2026-07-18-terminal-observability-event-archive-design.md) 冻结：collector 对已登记 request 与 logging/accounting child 的 admission 必须保持到本节 C step 7 `shutdown_dropped` settle 和 step 8 logging worker stop 之后；随后 flush worker IPC/spool、durable commit/ack terminal events并关闭 collector，最后才执行 step 9 的 aiohttp/prisma/redis teardown。两份 Spec 共用同一绝对 deadline，不能各开独立 cleanup 窗口。
