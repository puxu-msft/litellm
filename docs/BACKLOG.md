...
## Anthropic ↔ GPT 全保真格式转换（block 2/3/4 已完成）

来源: 2026-07-14 brainstorming「全面、全保真 Anthropic↔GPT 格式转换」。整体目标拆成四块，**block 1（reasoning↔thinking 全保真）已立项优先做**。block 2/3/4 已于 2026-07-17 启动，统一 spec 与计划见 `docs/superpowers/specs/2026-07-17-anthropic-protocol-tool-stream-fidelity-design.md` 和 `docs/superpowers/plans/2026-07-17-anthropic-protocol-tool-stream-fidelity.md`。判定标准仍是「gpt 对 Claude Code 表现得像原生 Anthropic 模型」。

- **block 2 — 协议信封正确性**: 修双 `message_start`、空壳 thinking、断流时 `message_delta`/`message_stop` 缺失等，让 gpt/Responses→Anthropic 的 SSE 事件序列严格合规。部分空壳 thinking 会被 block 1 顺带解决，其余留此块。相关观测见 `~/.config/litellm/docs/illformed-fix.md`
...
- **block 2 已完成**：Chat adapter 与 direct Responses adapter 的自然/异常 EOF 完整收尾；direct Responses fallback + `response.created` 双 `message_start` 修复；strict SDK、真实 Claude CLI、carrier replay/tamper/continuity live 门禁通过
- **block 3 已完成到协议可表示上限**：未知 `tool_result`、并行/交错 tool_calls、分片 arguments、`disable_parallel_tool_use` ↔ `parallel_tool_calls` 均有核心回归；模型业务 schema 补全留 hookpkg。`tool_result.is_error` 经 live PoC 证实无标准或私有无损载体，保留 content/id 并明确记录 error bit 有损
- **block 4 已完成**：direct Responses 主动规整 orphan delta/done、重复 start/done、负 index、未闭合块与断流信封；解析前 hook 与 keepalive 前 core 双边界重组均保留，hook 复用 core delimiter/上限 primitives，live wire 恰好一个 message_start 且完整终止

本节无剩余实现项；稳定行为见 `docs/superpowers/specs/2026-07-17-anthropic-protocol-tool-stream-fidelity-design.md` 与 `config/docs/illformed-fix.md`
...
现状: hook 层(`~/.config/litellm/hookpkg/stream.py` `stream_transform`)**无法**掐断到 copilot 的上游 HTTP 流——async generator 的 `aclose()` 不向下级联,整条包装链没有一层 `finally: await owner.aclose()`,`stream_transform` 里 `return`/`break` 只关客户端连接、不关上游(实测 `httpx_response_is_closed=False`,靠 GC/背压兜)。owner 是原生 anthropic-messages 路径的 `httpx.Response`,hook 够不到。
...
延后理由: 退化场景最终选「去重不掐断」(上游会自恢复、掐断丢有效数据),token 省不下的代价用户已接受;当前无其它取消需求。详见 `~/.config/litellm/docs/plan/degeneration-cutoff.md` 决策记录与该 skill 的 `reference/stream-cancellation-and-abort.md`。
